...
from conductor.service import result_upload_finished
...
class AuthGetTaskResultIntegrationTest(ConcentIntegrationTestCase):
    ...
    def test_concent_requests_task_result_from_provider_and_requestor_receives_failure_because_provider_does_not_finish_upload_should_work_only_with_correct_keys(self):
        ...
        self.assertIsInstance(message_file_transfer_token, message.concents.FileTransferToken)
        self.assertEqual(
            message_file_transfer_token.subtask_id,
            deserialized_task_to_compute.compute_task_def['subtask_id']
    ...
    def test_concent_requests_task_result_from_provider_and_requestor_receives_task_result_should_work_only_with_correct_keys(self):
        ...
        self.assertEqual(
            message_from_concent.force_get_task_result.report_computed_task.task_to_compute.compute_task_def['task_id'],
            deserialized_task_to_compute.task_id
        )
        self.assertEqual(
            message_from_concent.file_transfer_token.subtask_id,
            deserialized_task_to_compute.compute_task_def['subtask_id']
        )  # pylint: disable=no-member
