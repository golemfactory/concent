...
def if_given_version_of_golem_messages_is_compatible_with_version_in_concent(protocol_version: str) -> bool:
    """
    ...
    """
    for i in range(0, 2):
        if not protocol_version.split('.')[i] == settings.GOLEM_MESSAGES_VERSION.split('.')[i]:
            return False
    return True
