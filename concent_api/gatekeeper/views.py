...
def upload(request):
    ...
    if request.content_type != 'application/x-www-form-urlencoded':
        return gatekeeper_access_denied_response(
            ...
            ErrorCode.HEADER_CONTENT_TYPE_NOT_SUPPORTED.value,
        )
...
def download(request):
    ...
    if request.content_type != 'text/plain' and request.content_type != '':
        return gatekeeper_access_denied_response(
            ...
            ErrorCode.BODY_NOT_EMPTY.value,
        )
...
def parse_headers(request: WSGIRequest, path_to_file: str) -> Union[FileTransferToken.FileInfo, JsonResponse]:
    ...
    if scheme != 'Golem':
        return gatekeeper_access_denied_response(
            ...
            ErrorCode.HEADER_AUTHORIZATION_UNRECOGNIZED_SCHEME.value,
            path_to_file,
        )
    ...
    try:
        ...
        return gatekeeper_access_denied_response(
            ...
            ErrorCode.HEADER_AUTHORIZATION_UNABLE_TO_DECODE.value,
            path_to_file,
        )
    ...
    if 'HTTP_CONCENT_CLIENT_PUBLIC_KEY' not in request.META:
        return gatekeeper_access_denied_response(
            'Missing Concent-Client-Public-Key header.',
            ErrorCode.HEADER_CLIENT_PUBLIC_KEY_MISSING.value,
            path_to_file,
    ...
    if not isinstance(loaded_golem_message.token_expiration_deadline, int):
        return gatekeeper_access_denied_response(
            'Wrong type of token_expiration_deadline variable.',
            ErrorCode.MESSAGE_TOKEN_EXPIRATION_DEADLINE_WRONG_TYPE.value,
            path_to_file,
            loaded_golem_message.subtask_id,
            client_public_key,
        )
    ...
    if current_time > loaded_golem_message.token_expiration_deadline:
        return gatekeeper_access_denied_response(
            ...
            ErrorCode.MESSAGE_TOKEN_EXPIRATION_DEADLINE_PASSED.value,
            path_to_file,
            loaded_golem_message.subtask_id,
            client_public_key,
        )
    ...
    if not isinstance(loaded_golem_message.storage_cluster_address, str):
        return gatekeeper_access_denied_response(
            'Wrong type of storage_cluster_address variable.',
            ErrorCode.MESSAGE_STORAGE_CLUSTER_WRONG_TYPE.value,
            path_to_file,
            loaded_golem_message.subtask_id,
            client_public_key,
        )
    ...
    try:
        ...
        return gatekeeper_access_denied_response(
            ...
            ErrorCode.MESSAGE_STORAGE_CLUSTER_INVALID_URL.value,
            path_to_file,
            loaded_golem_message.subtask_id,
            client_public_key,
        )
    if loaded_golem_message.storage_cluster_address != settings.STORAGE_CLUSTER_ADDRESS:
        return gatekeeper_access_denied_response(
            'Given storage_cluster_address is not defined in settings.',
            ErrorCode.MESSAGE_STORAGE_CLUSTER_UNKNOWN.value,
            path_to_file,
            loaded_golem_message.subtask_id,
            client_public_key,
        )
    ...
    if not isinstance(loaded_golem_message.authorized_client_public_key, bytes):
        return gatekeeper_access_denied_response(
            'Wrong type of authorized_client_public_key variable.',
            ErrorCode.MESSAGE_AUTHORIZED_CLIENT_PUBLIC_KEY_WRONG_TYPE.value,
            path_to_file,
            loaded_golem_message.subtask_id,
            client_public_key,
        )
    ...
    if client_public_key != client_public_key_base64:
        return gatekeeper_access_denied_response(
            'authorized_client_public_key is different then Concent-Client-Public-Key header.',
            ErrorCode.MESSAGE_AUTHORIZED_CLIENT_PUBLIC_KEY_INVALID.value,
            path_to_file,
            loaded_golem_message.subtask_id,
            client_public_key,
        )
    ...
    if request.method == 'POST' and loaded_golem_message.operation != 'upload':
        return gatekeeper_access_denied_response(
            'Wrong operation value for this request method.',
    ...
    if not all(isinstance(file, dict) for file in loaded_golem_message.files):
        return gatekeeper_access_denied_response(
            'Wrong type of files variable.',
            ErrorCode.MESSAGE_FILES_WRONG_TYPE.value,
            path_to_file,
            loaded_golem_message.subtask_id,
            client_public_key
        )
    ...
    if len(transfer_token_paths_to_files) != len(set(transfer_token_paths_to_files)):
        return gatekeeper_access_denied_response(
            ...
            ErrorCode.MESSAGE_FILES_PATH_NOT_UNIQUE.value,
            path_to_file,
            loaded_golem_message.subtask_id,
            client_public_key,
        )
    ...
    if not all(isinstance(file["checksum"], str) for file in loaded_golem_message.files):
        return gatekeeper_access_denied_response(
            ...
            ErrorCode.MESSAGE_FILES_CHECKSUM_WRONG_TYPE.value,
            path_to_file,
            loaded_golem_message.subtask_id,
            client_public_key,
        )
    if any((len(file["checksum"]) == 0 or file["checksum"].isspace()) for file in loaded_golem_message.files):
        return gatekeeper_access_denied_response(
            ...
            ErrorCode.MESSAGE_FILES_CHECKSUM_EMPTY.value,
            path_to_file,
            loaded_golem_message.subtask_id,
            client_public_key,
        )
    if not all(":" in file["checksum"] for file in loaded_golem_message.files):
        return gatekeeper_access_denied_response(
            ...
            ErrorCode.MESSAGE_FILES_CHECKSUM_WRONG_FORMAT.value,
            path_to_file,
            loaded_golem_message.subtask_id,
            client_public_key,
        )
    ...
    if not set(checksum_type for checksum_type, checksum in file_checksums).issubset(set(HashingAlgorithm._value2member_map_)):  # type: ignore
        return gatekeeper_access_denied_response(
            ...
            ErrorCode.MESSAGE_FILES_CHECKSUM_UNRECOGNIZED_TYPE.value,
            path_to_file,
            loaded_golem_message.subtask_id,
            client_public_key,
        )
    ...
    if any(VALID_SHA1_HASH_REGEX.match(file_checksum[1]) is None for file_checksum in file_checksums):
        return gatekeeper_access_denied_response(
            "Invalid SHA1 hash.",
            ErrorCode.MESSAGE_FILES_CHECKSUM_INVALID_SHA1_HASH.value,
            path_to_file,
            loaded_golem_message.subtask_id,
            client_public_key,
        )
    ...
    if any((file_size is None) for file_size in file_sizes):
        return gatekeeper_access_denied_response(
            ...
            ErrorCode.MESSAGE_FILES_SIZE_WRONG_TYPE.value,
            path_to_file,
            loaded_golem_message.subtask_id,
            client_public_key,
        )
    ...
    if any(int(file["size"]) < 0  for file in loaded_golem_message.files):
        return gatekeeper_access_denied_response(
            ...
            ErrorCode.MESSAGE_FILES_SIZE_NEGATIVE.value,
            path_to_file,
            loaded_golem_message.subtask_id,
            client_public_key,
        )
    ...
    if len(matching_files) == 1:
        ...
        return gatekeeper_access_denied_response(
            'Path to specified file is not listed in files variable.',
            ErrorCode.MESSAGE_FILES_PATH_NOT_LISTED_IN_FILES.value,
            path_to_file,
            loaded_golem_message.subtask_id,
            client_public_key,
        )
