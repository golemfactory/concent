...
#### Sequence 2: client A requests additional verification of subtask S11 rejected by client D
All sequences of events happen simultaneously.
They're separated only for readability.

- **[t=500]** Client A agrees to compute subtask S10 for client D.
    - `TaskToCompute.timestamp`: 500
    - `TaskToCompute.deadline`: 600

- **[t=700]** Client D sends `SubtaskResultsRejected` directly to client A.

- **[t=900]** Client A submits `SubtaskResultVerify` message to `send/` endpoint.
    - Task parameters:
        - subtask_id: S11
        - total price (`TaskToCompute.price` * task duration): 8 GNT
        - requestor's Ethereum account: D1
        - provider's Ethereum account: A1
    - **[t=1002]** Concent validates the message and determines that the request is valid.
    - **[t=1003]** Concent makes a HTTP request to `bankster/claim-deposit/` endpoint
        - `concent_use_case`: `AdditionalVerification`
        - `subtask_cost`: 8 GNT
